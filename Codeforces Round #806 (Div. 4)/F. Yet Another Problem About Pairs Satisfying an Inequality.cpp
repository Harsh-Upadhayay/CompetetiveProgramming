...
void solve(void){
    ll n, ans = 0; cin >> n;
    vll v(n+1); vector<p(ll, ll)> pairs; rpt(i, 1, n+1) {cin >> v[i]; if(v[i] < i) pairs.pb({v[i], i});}
    sort(all(pairs));
    debug(pairs);

    for(auto x : pairs)
        ans += (long long)(upper_bound(pairs.begin(), pairs.end(), x) - pairs.begin());
