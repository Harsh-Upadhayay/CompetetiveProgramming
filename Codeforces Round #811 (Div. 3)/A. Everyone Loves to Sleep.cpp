...
ll diff(ll H, ll M, ll h, ll m) {
    ll sleepTime = H * 60 + M, upTime = h * 60 + m;
    if(sleepTime <= upTime)
        return upTime - sleepTime;
    else
        return 24 * 60 - (sleepTime - upTime);
}
...
void solve(void){
    ...
    rpt(i, 0, n) {
        ll h, m; cin >> h >> m;
        minTime = min(diff(H, M, h, m), minTime);
    }
    cout << minTime / 60 << " " << minTime % 60;
    nl;
}
